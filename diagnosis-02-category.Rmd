...
- by using a number, I can then sum across rows to get totals. I have to use `rowwise()` to do this.
- by having the categorization, I could do that using `starts_with("mb_")`, etc.


## Questions

- If `id` is a unique childe, then some of these are the same kids abandoned more than once.
...
The idea here is to create new columns (with structured names) that has "1" if any diagnosis is present in that column. I do this by setting to "1" if not NA.
...
Creates `diag_count` columns sum total diagnosis = true and category = true.
...
abandon_cats_totaled <- abandon_cats %>% 
  ...
  mutate(
    ...
    diag_count_total = sum(c_across(starts_with(c("a_","b_", "c_")))),
  )
```
...
Just filters for totals more than 1 then counts the rows.

```{r count_diag_gt_two}
abandon_cats_totaled %>% 
  filter(diag_count_total >= 2) %>% 
  nrow()
```

### Testing diagnosis counts

This shows proof of concept that the diag_counts work.
